...
@app.route('/api/public/power/v1/tasks/<task_uuid>')
...
@app.route('/api/public/power/v1/on', methods=['POST'])
def handle_poweron():
    data = json.loads(flask.request.get_data().decode())
    name = f'power-off for {data["cluster"]["name"]}'
    task_uuid = str(uuid.uuid4())
    parent_task_uuid = data['parent_uuid'] if 'parent_uuid' in data else ''

    json_string = json.dumps(data)
    t = threading.Thread(target=do_action, args=(name, task_uuid, parent_task_uuid, POWERON_COMMAND, json_string))
    t.start()
...
@app.route('/api/public/power/v1/off', methods=['POST'])
def handle_poweroff():
    data = json.loads(flask.request.get_data().decode())
    name = f'power-on for {data["cluster"]["name"]}'
    task_uuid = str(uuid.uuid4())
    parent_task_uuid = data['parent_uuid'] if 'parent_uuid' in data else ''

    json_string = json.dumps(data)
    t = threading.Thread(target=do_action, args=(name, task_uuid, parent_task_uuid, POWEROFF_COMMAND, json_string))
    t.start()
    return flask.jsonify({
            'uuid': task_uuid,
            'task_url': f'http://{HOST}:{PORT}/api/public/foundation/v1/tasks/{task_uuid}'
...
def do_action(name, task_uuid, parent_task_uuid, command, json_string):
    # prepare
    tasks[task_uuid] = {
        'name': name,
        'uuid': task_uuid,

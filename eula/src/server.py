...
# params
HOST = os.environ['API_EULA_HOST']
PORT = int(os.environ['API_EULA_PORT'])
DEBUG = True
COMMAND = "python3 -u eula.py -j '{}'"

# app and state
app = flask.Flask('API EULA')
tasks = OrderedDict()


@app.route('/api/public/eula/v1/tasks/')
def handle_tasks():
    # clean up tasks
    now = int(time.time())
    del_uuids = []
...
@app.route('/api/public/eula/v1/tasks/<task_uuid>')
...
@app.route('/api/public/eula/v1/run', methods=['POST'])
def handle_run():
    data = json.loads(flask.request.get_data().decode())
    name = f'eula for {data["cluster"]["name"]}'
    task_uuid = str(uuid.uuid4())
    parent_task_uuid = data['parent_uuid'] if 'parent_uuid' in data else ''


    json_string = json.dumps(data)
    t = threading.Thread(target=do_eula, args=(name, task_uuid, parent_task_uuid, json_string))
    t.start()
    return flask.jsonify({
            'uuid': task_uuid,
            'task_url': f'http://{HOST}:{PORT}/api/public/eula/v1/tasks/{task_uuid}'
        })


def do_eula(name, task_uuid, parent_task_uuid, json_string):
    # prepare
    tasks[task_uuid] = {
        'name': name,
        'uuid': task_uuid,
